...
def tokenize_text(text: str) -> tuple[list[str], dict, dict]:
    ...
    vocab = sorted(token_counts, key=token_counts.__getitem__, reverse=True)
...
def create_sequences(tokens: list[str], token_to_int: dict, seq_length = STANDARD_SEQUENCE_LENGTH) -> tuple[np.ndarray, np.ndarray]:
    sequences = []
    targets = []
...
class TextGenerationModel(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int, hidden_dim: int, n_layers: int):
        ...
        of the network. Updating these values depends on the activation function, which in this case is the Tanh
        function. The new weight of a node is highly dependent on the gradient of the loss function, which is in turn dependent
        on the partial derivative of the activaiton function. With a Tanh activation function, the partial derivative
        continues to get smaller for each layer it back propagates, resulting in a loss function that approaches zero.
        
        The solution to this is to use an LSTM (Long Short-Term Memory), which introduces a memory cell that holds information 
    ...
    hidden: A tuple of (h_0, c_0) — the initial hidden state and cell state for the LSTM.
...
def train_model(model, data_loader, epochs=EPOCHS, lr=LR):
    # Explicitly sets the model to training mode
    model.train()
